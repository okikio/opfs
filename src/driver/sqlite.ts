import { createDb0Driver, type Db0PrimitiveType, type Db0StatementType } from "./db0.ts";
import type { RecordDriverType } from "./record.ts";
...
/** Minimal connected SQLite database contract used by the record driver. */
...
/** Direct SQLite record-driver options. */
export interface SqliteDriverOptionsType {
  /** Driver-owned table. Defaults to `opfs_entries`. */
  readonly table?: string;
  /** Creates the table before returning. Defaults to true. */
  readonly initialize?: boolean;
  /** Closes the injected database with the driver. */
...
class SqliteDatabase {
  /** db0 dialect identity consumed by the shared SQL record driver. */
...
/** Creates a record driver from a connected SQLite database. */
export async function createSqliteDriver(
  database: SqliteDatabaseType,
  options: SqliteDriverOptionsType = {},
): Promise<RecordDriverType> {
  return await createDb0Driver(new SqliteDatabase(database, options.disposeDatabase ?? false), {
    ...(options.table === undefined ? {} : { table: options.table }),
    ...(options.initialize === undefined ? {} : { initialize: options.initialize }),
    disposeDatabase: options.disposeDatabase ?? false,
  });
}

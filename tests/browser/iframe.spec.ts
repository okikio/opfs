...
/** File-local Window shape while the iframe fixture module may still be initializing. */
...
async function waitForApi(frame: import("@playwright/test").Frame): Promise<void> {
  await frame.waitForFunction(() => Boolean((window as PendingFixtureWindowType).opfsTest?.ready));
}
